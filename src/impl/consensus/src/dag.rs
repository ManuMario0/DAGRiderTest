...
impl Dag {
    ...
    pub fn contains_vertices(&self, vertices: &BTreeMap<VertexHash, Round>) -> bool {
        vertices
            .iter()
            .all(|(vertex_hash, round)| match self.graph.get(round) {
    ...
    pub fn is_strongly_linked(&self, newest: &Vertex, oldest: &Vertex) -> bool {
        self.is_linked_internal(
            newest,
            oldest,
            |v: &Vertex| -> BTreeMap<VertexHash, Round> { v.get_strong_parents() },
        )
    }
    ...
    pub fn is_linked(&self, newest: &Vertex, oldest: &Vertex) -> bool {
        self.is_linked_internal(
            newest,
            oldest,
            |v: &Vertex| -> BTreeMap<VertexHash, Round> { v.get_all_parents() },
        )
    }

    fn is_linked_internal(
        &self,
        newest: &Vertex,
        oldest: &Vertex,
        get_parents: fn(&Vertex) -> BTreeMap<VertexHash, Round>,
    ) -> bool {
