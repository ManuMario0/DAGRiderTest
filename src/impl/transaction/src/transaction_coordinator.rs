...
impl TransactionCoordinator {
    pub fn spawn(node_id: Id, committee: Committee, block_sender: Sender<Block>) {
        let (transaction_to_block_builder_sender, transaction_receiver) =
        ...
        Receiver::spawn(
            tx_address,
            TxReceiverHandler {
                transaction_to_block_builder_sender,
            },
        );

        let address = committee.get_block_receiver_address(node_id).unwrap();
        debug!("Start listening for blocks on {:?}", address);
        Receiver::spawn(address, BlockReceiverHandler { block_sender });

        BlockBuilder::spawn(transaction_receiver, committee);
    }
}
...
impl MessageHandler for TxReceiverHandler {
    async fn dispatch(&self, _writer: &mut Writer, message: Bytes) -> Result<(), Box<dyn Error>> {
        info!(
            "TxReceiverHandler received transaction to process {:?}",
            message
        );
